...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		$data['sort_customer_code'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=customer_code' . $url, true);
		$data['sort_name'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=customer_name' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->error['status'])) {
			$data['error_customer_status'] = $this->error['status'];
		} else {
			$data['error_customer_status'] = '';
		}

		if (isset($this->error['telephone'])) {
			$data['error_customer_telephone'] = $this->error['telephone'];
		} else {
			$data['error_customer_telephone'] = '';
		}

		if (isset($this->error['ext'])) {
			$data['error_customer_ext'] = $this->error['ext'];
		} else {
			$data['error_customer_ext'] = '';
		}
		
		if (isset($this->error['fax'])) {
			$data['error_customer_fax'] = $this->error['fax'];
		} else {
			$data['error_customer_fax'] = '';
		}
		...
		if (isset($this->error['user_phone'])) {
			$data['error_user_phone'] = $this->error['user_phone'];
		} else {
			$data['error_user_phone'] = '';
		}
		if (isset($this->error['user_res_phone'])) {
			$data['error_user_res_phone'] = $this->error['user_res_phone'];
		} else {
			$data['error_user_res_phone'] = '';
		}	
		if (isset($this->error['user_cell_phone'])) {
			$data['error_user_cell_phone'] = $this->error['user_cell_phone'];
		} else {
			$data['error_user_cell_phone'] = '';
		}	
		if (isset($this->error['user_ext'])) {
			$data['error_user_ext'] = $this->error['user_ext'];
		} else {
			$data['error_user_ext'] = '';
		}	
		...
		if(isset($this->request->get['customer_id'])) {
			$customer_user = $this->model_customer_customer->getCustomerUser($this->request->get['customer_id']);
			//var_dump($customer_user);
		}
		...
		$data['job_titles'] = $this->model_customer_customer->getJobTitle();

		if(isset($this->request->post['title']) && !empty($this->request->post['title'])) {
			$data['user_title'] = $this->request->post['title'];
		} else {
			$data['user_title'] = '';
		}
		if(isset($this->request->post['first_name']) && !empty($this->request->post['first_name'])) {
			$data['user_first_name'] = $this->request->post['first_name'];
		} else {
			$data['user_first_name'] = '';
		}
		if(isset($this->request->post['last_name']) && !empty($this->request->post['last_name'])) {
			$data['user_last_name'] = $this->request->post['last_name'];
		} else {
			$data['user_last_name'] = '';
		}
		if(isset($this->request->post['user_phone']) && !empty($this->request->post['user_phone'])) {
			$data['user_phone'] = $this->request->post['user_phone'];
		} else {
			$data['user_phone'] = '';
		}
		if(isset($this->request->post['user_ext']) && !empty($this->request->post['user_ext'])) {
			$data['user_ext'] = $this->request->post['user_ext'];
		} else {
			$data['user_ext'] = '';
		}
		if(isset($this->request->post['user_res_phone']) && !empty($this->request->post['user_res_phone'])) {
			$data['user_res_phone'] = $this->request->post['user_res_phone'];
		} else {
			$data['user_res_phone'] = '';
		}
		if(isset($this->request->post['user_cell_phone']) && !empty($this->request->post['user_cell_phone'])) {
			$data['user_cell_phone'] = $this->request->post['user_cell_phone'];
		} else {
			$data['user_cell_phone'] = '';
		}
		if(isset($this->request->post['job_title']) && !empty($this->request->post['job_title'])) {
			$data['user_job_title'] = $this->request->post['job_title'];
		} else {
			$data['user_job_title'] = '';
		}
		if(isset($this->request->post['user_email']) && !empty($this->request->post['user_email'])) {
			$data['user_email'] = $this->request->post['user_email'];
		} else {
			$data['user_email'] = '';
		}
		
		if(isset($customer_user) && !empty($customer_user)) {
			$data['users_present'] = true;
			$data['customer_users'] = $customer_user;
		}	else {
			$data['users_present'] = false;
		}

		if (isset($this->request->post['customer_users'])) {
			$data['customer_users_edit'] = $this->request->post['customer_users'];
		} else {
			$data['customer_users_edit'] = '';
		}
		...
		if (isset($this->request->post['telephone'])) {
			...
			$data['telephone'] = $this->model_tool_phone_helper->formatPhoneNumber($customer_info['telephone']);
		} else {
			$data['telephone'] = '';
		}
		...
		if (isset($this->request->post['fax'])) {
			...
			$data['fax'] = $this->model_tool_phone_helper->formatPhoneNumber($customer_info['fax']);
		} else {
			$data['fax'] = '';
		}
		...
		if (isset($this->request->post['address'])) {
			...
			$data['addresses'] = $this->model_customer_customer->getCustomerAddresses($this->request->get['customer_id']);
		} else {
		...
		if (isset($this->request->post['status'])) {
			...
			$data['cust_status_disabled'] = true;
		} else {
			$data['status'] = false;
		}
		...
		$this->document->addStyle('https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/css/select2.min.css');
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
	...
	public function getUserDetails() {
		$user_id = $this->db->escape($this->request->post['user_id']);

		$this->load->model('customer/customer');
		echo json_encode($this->model_customer_customer->getUserById($user_id)); die;
	}
	...
	protected function validateForm(string $type) {
		...
		if($type == 'add' && (!isset($this->request->post['customer_code']) || empty($this->request->post['customer_code']))) {
			$this->error['customer_code'] = 'Customer code field is compulsory';
		}
		if($type == 'add' && (!isset($this->request->post['customer_name']) || empty($this->request->post['customer_name']))) {
			$this->error['customer_name'] = 'Customer name field is compulsory';
		}
		if($type == 'add' && isset($this->request->post['status']) && !in_array($this->request->post['status'], ["0", "1"])) {
			$this->error['status'] = 'Select valid customer status';
		}
		if (isset($this->request->post['telephone']) && !empty($this->request->post['telephone']) && !$this->model_tool_phone_helper->validatePhoneNumber($this->request->post['telephone'])) {
			$this->error['telephone'] = 'Please enter valid telephone number';
		}
		if (isset($this->request->post['ext']) && !empty($this->request->post['ext']) && (utf8_strlen($this->request->post['ext']) > 6)) {
			$this->error['ext'] = 'Invalid extension';
		}
		if (isset($this->request->post['fax']) && !empty($this->request->post['fax']) && !$this->model_tool_phone_helper->validatePhoneNumber($this->request->post['fax'])) {
			$this->error['fax'] = 'Please enter valid fax';
		}

		//Validation for customer main contact
		if((!isset($this->request->post['customer_users'])) && empty($this->request->post['first_name'])) {
			$this->error['first_name'] = 'Firstname field is compulsory';
		} else {
			if(isset($this->request->post['first_name']) && utf8_strlen($this->request->post['first_name']) < 3 && utf8_strlen($this->request->post['first_name']) > 50) {
				$this->error['first_name'] = 'Enter valid firstname';
			}
		}
		if((!isset($this->request->post['customer_users'])) && empty($this->request->post['last_name'])) {
			$this->error['last_name'] = 'Lastname field is compulsory';
		} else {
			if(isset($this->request->post['last_name']) && utf8_strlen($this->request->post['last_name']) < 3 && utf8_strlen($this->request->post['last_name']) > 50) {
				$this->error['last_name'] = 'Enter valid Lastname';
			}
		}
		if(empty($this->request->post['user_phone'])) {
			$this->error['user_phone'] = 'Phone number field is compulsory';
		} else {
			if(!$this->model_tool_phone_helper->validatePhoneNumber($this->request->post['user_phone'])) {
				$this->error['user_phone'] = 'Enter valid phone number';
			}
		}
		if(!empty($this->request->post['user_res_phone']) && !$this->model_tool_phone_helper->validatePhoneNumber($this->request->post['user_res_phone'])) {
			$this->error['user_res_phone'] = 'Enter valid phone number';
		}
		if(!empty($this->request->post['user_cell_phone']) && !$this->model_tool_phone_helper->validatePhoneNumber($this->request->post['user_cell_phone'])) {
			$this->error['user_cell_phone'] = 'Enter valid phone number';
		}
		if(empty($this->request->post['user_email'])) {
			$this->error['user_email'] = 'Email address field is compulsory';
		} else {
			if(!filter_var($this->request->post['user_email'], FILTER_VALIDATE_EMAIL)) {
				$this->error['user_email'] = 'Enter valid email address';
			}
		}
		if (isset($this->request->post['user_ext']) && (utf8_strlen($this->request->post['user_ext']) > 6)) {
			$this->error['user_ext'] = 'Invalid extension';
		}

		//Validation for customer addresses
		if (isset($this->request->post['address'])) {
			foreach ($this->request->post['address'] as $key => $value) {

				if (isset($value['description']) && (utf8_strlen($value['description']) > 128)) {
					$this->error['address'][$key]['description'] = 'Description too long (cannot be greater than 128 characters)';
				}

				if (isset($value['firstname']) && (utf8_strlen($value['firstname']) > 50)) {
					$this->error['address'][$key]['firstname'] = 'Firstname too long (cannot be greater than 50 characters)';
				}

				if (isset($value['lastname']) && (utf8_strlen($value['lastname']) > 50)) {
					$this->error['address'][$key]['lastname'] = 'Lastname too long (cannot be greater than 50 characters)';
				}

				if (isset($value['company']) && (utf8_strlen($value['company']) > 128)) {
					$this->error['address'][$key]['company'] = 'Company too long (cannot be greater than 128 characters)';
				}
				...
				if(isset($value['postcode']) && !empty($value['postcode']) && !$this->model_tool_phone_helper->validatePostalCode($value['postcode'])) {
					$this->error['address'][$key]['postcode'] = 'Enter valid postal code';
				}

				if (!isset($value['country_id']) || empty($value['country_id'])) {
					$this->error['address'][$key]['country_id'] = 'Select country from drop down';
				}

				if (!isset($value['zone_id']) || empty($value['zone_id'])) {
					$this->error['address'][$key]['zone_id'] = 'Select region from drop down';
				}
