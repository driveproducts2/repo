...
class ControllerCustomerCustomerApproval extends Controller {
	...
	public function approve() {
		...
		if (!$this->user->hasPermission('modify', 'customer/customer_approval')) {
			...
			$rand_str = $this->model_customer_customer_approval->generateRndStr();
			//$password = $this->db->escape(sha1($salt . sha1($salt . sha1($rand_str))));

			if ($this->request->get['type'] == 'customer') {
				$this->model_customer_customer_approval->approveCustomer($this->request->get['customer_id'], $salt, $rand_str);
