	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			if(isset($result['model']) && !empty($result['model'])) {
			 	$product_price_res = $this->model_catalog_product->calculateProductsBasePrice([$result['model']]);
				if (isset($product_price_res[$result['model']]['base_price'])) {
					$product_price = $product_price_res[$result['model']]['base_price'];
				} else {
					$product_price = 0;
				}
			} else {
			...
			$data['products'][] = array(
				...
				'price'      => $this->currency->format($product_price, $this->config->get('config_currency')),
				'special'    => $special,
