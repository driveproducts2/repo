...
class ModelCustomerCustomerApproval extends Model {
	...
	public function approveCustomer($customer_id, $salt, $rand_str) {
		//var_dump($customer_id, $salt, $pwd); die;
		$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET status = '1', salt = '".$salt."', password = '".$this->db->escape(sha1($salt . sha1($salt . sha1($rand_str))))."' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function generateRndStr($length = 12) {
