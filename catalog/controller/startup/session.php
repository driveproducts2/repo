...
class ControllerStartupSession extends Controller {
	public function index() {
		if (isset($this->request->get['api_token']) && isset($this->request->get['route']) && substr($this->request->get['route'], 0, 4) == 'api/') {
			...
			// Check if "Stay Logged In" cookies are present
			if (isset($_COOKIE['customer_id']) && !empty($_COOKIE['customer_id'])) {
				$this->load->model('account/customer');
				$customer_info = $this->model_account_customer->getCustomer($_COOKIE['customer_id']);

				if ($customer_info && $_COOKIE['customer_id'] == $customer_info['customer_id']) {
					// Regenerate session data for the user
