	...
	public function index() {
		...
		if (!$redirect) {
			...
			if ($this->cart->hasShipping()) {
				...
				$order_data['shipping_address_code'] = $this->session->data['shipping_address']['address_code'];
			...
			$model_det_arr = [];
			$all_products_det = $this->cart->getProducts();

			foreach ($all_products_det as $key => $value) {
				$model = $value['model'];
				$model_det_arr[$model]['key'] = $key;
				$model_det_arr[$model]['quantity'] = $value['quantity'];
			}

			$models = array_keys($model_det_arr);
			$customerGroup = isset($this->session->data['customer_code']) ? $this->session->data['customer_code'] : '';

			// Create a session key based on model list + customer group
			$cacheKey = 'net_price_' . md5(json_encode($models) . $customerGroup);

			// Check if net prices are cached in session
			if (isset($this->session->data[$cacheKey])) {
				$price = $this->session->data[$cacheKey];
			} else {
				// Call the heavy method and cache the result
				$price = $this->model_catalog_product->calculateProductsNetPrice($models, $customerGroup);
				$this->session->data[$cacheKey] = $price;
			}

			// Assign prices and totals to product list
			foreach ($model_det_arr as $k => $v) {
				$all_products_det[$v['key']]['price'] = $price[$k]['net_price'];
				$all_products_det[$v['key']]['total'] = $price[$k]['net_price'] * $v['quantity'];
			}
			//echo "<pre>"; print_r($all_products_det); die;

			foreach ($all_products_det as $product) {
				$option_data = array();
			...
			$order_data['comment'] = $this->session->data['shipping_instruction'];
			$order_data['purchase_order'] = isset($this->session->data['purchase_order']) ? $this->session->data['purchase_order'] : '';
