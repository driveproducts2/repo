...
class ControllerApiOrder extends Controller {
	...
	/*Order from Dynamics GP related function here*/
  ...
  private function authenticate()
  {
    ...
    $result = ($this->model_account_order->insertOrdersFromGP());
		var_dump($result); die;
	...
	//Get orders from webstore tables
  public function syncOrdersFromWebstore(): void
  {
    try {
      ...
      Logger::log("Fetching orders from webstore", "INFO");
      $res_data = $this->model_api_order->getOrdersFromWebstore();

      //Logger::log("Order data fetched : ".json_encode($res_data), "INFO");
      http_response(200, ["res" => $res_data]);
	...
	//Update sync orders from GP
  public function syncUpdateWebordFromGP(): void
  {
    try {
      $this->load->model('api/order');
      $this->load->helper('logger');
      ...
      Logger::log("Started sync for : Update sync orders from GP", "INFO");
			//Logger::log("Result : ".json_encode($data)); die;
      $inserted = $this->model_api_order->updateSyncedOrdersToWebstore($data['data']);

      Logger::log("Total records affected in order tables in webstore database : ".$inserted, "INFO");
