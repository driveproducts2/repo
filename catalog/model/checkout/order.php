...
class ModelCheckoutOrder extends Model {
	...
	public function calculateTotalTax(float $sub_total, int $zone_id, $country_id): array {
		$total_tax = 0;
		$result = [];
		$tax_det_arr = [];
		
		$query = $this->db->query("SELECT geo_zone_id FROM ".DB_PREFIX."zone_to_geo_zone WHERE country_id = '".$country_id."' AND zone_id = '".$zone_id."'");

		if($query->num_rows > 0) {
			$zone_id_str = '';
			foreach ($query->rows as $geo_zone) {
				foreach($geo_zone as $zone_id) {
					$zone_id_str .= $zone_id.',';
				}
			}
			$zone_id_str = rtrim($zone_id_str, ',');

			if($zone_id_str != '') {
				$query2 = $this->db->query("SELECT `name`, `rate`, `type` FROM `oc_tax_rate` WHERE `geo_zone_id` IN (".$zone_id_str.")");
				
				if($query2->num_rows > 0) {
					foreach($query2->rows as $tax_rates) {
						if($tax_rates['type'] == 'P') {
							$tax_price = round($sub_total * ($tax_rates['rate'] / 100), 2); 
						} elseif($tax_rates['type'] == 'F') {
							$tax_price = round($tax_rates['rate'], 2);
						}
						$total_tax += $tax_price;
						$tax_det_arr[] = ["name" => $tax_rates['name'], "type" => $tax_rates['type'], "rate" => $tax_rates['rate'], "tax_price" => $tax_price];
					}
				}
			}
		}
		$result = ["total_tax" => $total_tax, "tax_applied" => $tax_det_arr];
		return $result;
	}
}